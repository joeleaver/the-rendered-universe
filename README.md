...
**Eighteen runnable experiments on one hypothesis: the physical universe is not
...
Part 19 runs offline from the committed [`data/realsky_alm.npz`](data/realsky_alm.npz)
(100 KB); to rebuild it from the raw Planck/WMAP archives (~7 GB, URLs and
SHA-256 recorded inside), run `python3 tools/fetch_sky.py`.
...
observatory/  research instruments: causal geometry, Gaussian-state
              ...
              FITS/HEALPix reading (hand-rolled, validated at import)
data/         the real sky, reduced: a_lm of Planck 2018 x4 + WMAP9
              with URL + SHA-256 provenance (raw FITS not committed)
